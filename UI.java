...
public class UI {
	...
	public void initUI() {
		...
		mainFrame.setMinimumSize(new Dimension(1240, 800));
		mainFrame.setPreferredSize(new Dimension(1240, 800));
		...
		if (!forPlayer) {
			...
			for (int j = 0; j < cardsBeingPlayed.size(); j++) {
				if (playerCardContainers.containsKey(cardsBeingPlayed.get(j))) {
					System.out.println("removing the card number " + cardsBeingPlayed.get(j) + " : " + playerCardContainers.get(cardsBeingPlayed.get(j)));
